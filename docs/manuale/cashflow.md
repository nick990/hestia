...
Nella **categoria** puoi cercare. All’inizio vedi solo i primi livelli (casa, lavoro, …), in ordine alfabetico. Aprendo un gruppo vedi il secondo livello (`mutuo`, `monade`); se anche quello ha figli, si apre ancora e sotto trovi il resto (`stipendio`). Tocca il nome per assegnare quel livello (`lavoro` o `lavoro.monade`); la freccia apre e chiude. Senza una scelta resta «Nessuna».
